...
var TextGeneratorPlugin = class extends import_obsidian.Plugin {
  getGeneratedText(params) {
    return __async(this, null, function* () {
      ...
      const requestResults = JSON.parse(yield (0, import_obsidian.request)(reqParams));
...
`;
  ...
  insertGeneratedText(text, editor) {
  ...
  complete(params, insertMetadata = false, editor) {
    return __async(this, null, function* () {
      ...
      if (insertMetadata) {
        ...
        if (metadata.length === 0) {
          new import_obsidian.Notice("Please provide a valid frontmatter!");
          return;
        } else {
          params.prompt = this.getMetaData() + params.prompt;
        }
      }
      let text = yield this.getGeneratedText(params);
    ...
    return __async(this, null, function* () {
      ...
      const ribbonIconEl = this.addRibbonIcon("pencil_icon", "Generate Text!", (evt) => __async(this, null, function* () {
        ...
        if (activeView !== null) {
          ...
          yield this.complete(this.settings, false, editor);
      ...
      this.addCommand({
        ...
        editorCallback: (editor) => __async(this, null, function* () {
          this.updateStatusBar(`processing... `);
          yield this.complete(this.settings, true, editor);
          this.updateStatusBar(``);
        })
      });
      this.addCommand({
...
var TextGeneratorSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("api_key").setDesc("api_key").addText((text) => text.setPlaceholder("Enter your api_key").setValue(this.plugin.settings.api_key).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian.Setting(containerEl).setName("max_tokens").setDesc("max_tokens").addText((text) => text.setPlaceholder("max_tokens").setValue(this.plugin.settings.max_tokens.toString()).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.max_tokens = parseInt(value);
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("temperature").setDesc("temperature").addText((text) => text.setPlaceholder("temperature").setValue(this.plugin.settings.temperature.toString()).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.temperature = parseFloat(value);
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("frequency_penalty").setDesc("frequency_penalty").addText((text) => text.setPlaceholder("frequency_penalty").setValue(this.plugin.settings.frequency_penalty.toString()).onChange((value) => __async(this, null, function* () {
